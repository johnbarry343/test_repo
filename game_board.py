...
class GameBoard:
    ...
    def display(self, current_block=None):
        ...
        # This method is kept for basic debugging but not used for the main UI.
        # The main UI is generated by get_display_output_string().
        temp_grid = [row[:] for row in self.grid]
        if current_block:
            for r, row_data in enumerate(current_block.shape):
                for c, cell_val in enumerate(row_data):
                    if cell_val:
                        if 0 <= current_block.y + r < self.height and \
                           0 <= current_block.x + c < self.width:
                            temp_grid[current_block.y + r][current_block.x + c] = '■'

        for row in temp_grid:
            print(" ".join(['.' if cell == 0 else 'X' if cell == 1 else str(cell) for cell in row]))
        print("\n")
